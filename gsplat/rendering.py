...
def rasterization(
    ...
    if packed:
        ...
        # anomaly_test = anomaly_test[gaussian_ids]  #@@@@@@@@@@@ need to be removed later
    ...
    meta.update(
        {
            ...
            "conics": conics,
            "opacities": opacities,
            # "anomaly_test" : anomaly_test,#@@@@@@@@@@@@@@@@@@@@@@@@@@ need to be removed later
        }
    )
...
def custom_rasterization(
    ...
    width: int,
    height: int,
    anomals: Tensor,# @@@@need to be removed later
    near_plane: float = 0.01,
    far_plane: float = 1e10,
    radius_clip: float = 0.0,
    eps2d: float = 0.3,
    ...
    if packed:
        ...
        opacities = opacities[gaussian_ids]  # [nnz]
        anomals = anomals[gaussian_ids]  # Remove this line if anomaly_test is not defined
    ...
    meta.update(
        {
            ...
            "conics": conics,
            "opacities": opacities,
            "anomals": anomals,  # Remove this line if anomaly_test is not defined
        }
    )
    ...
    if sh_degree is None:
        ...
        if packed:
            ...
            masks = radii > 0  # [nnz]
            if colors.dim() == 3:
                # Turn [N, K, 3] into [nnz, K, 3]
                shs = colors[gaussian_ids, :, :]  # [nnz, K, 3]
            else:
                # Turn [C, N, K, 3] into [nnz, K, 3]
                shs = colors[camera_ids, gaussian_ids, :, :]  # [nnz, K, 3]
            colors = spherical_harmonics(sh_degree, dirs, shs, masks=masks)  # [nnz, 3]
        else:
            dirs = means[None, :, :] - camtoworlds[:, None, :3, 3]  # [C, N, 3]
