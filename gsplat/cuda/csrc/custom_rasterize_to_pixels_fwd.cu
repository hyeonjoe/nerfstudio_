...
__global__ void custom_rasterize_to_pixels_fwd_kernel(
    const uint32_t C,
    const uint32_t N,
    ...
    render_anomals += camera_id * image_height * image_width; //need to be removed later
    ...
    if (masks != nullptr && inside && !masks[tile_id]) {
        ...
        render_anomals[pix_id] = 0; //need to be removed later
        return;
    }
    ...
    bool has_anomal = false;            // need to be removed later
    int anomals_count = 0;              // need to be removed later
    ...
    if (inside) {
        ...
        // need to be removed later
        render_anomals[pix_id] = has_anomal ? 1 : 0;
        // need to be removed later
    }
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> call_kernel_with_dim(
    ...
    const torch::Tensor &anomals,                   // need to be removed later
    // image size
    const uint32_t image_width,
    const uint32_t image_height,
    const uint32_t tile_size,
...
rasterize_to_pixels_fwd_tensor(
    ...
    const at::optional<torch::Tensor> &backgrounds, // [C, channels]
    const at::optional<torch::Tensor> &masks, // [C, tile_height, tile_width]
    const torch::Tensor &anomals, // [nnz] or [C, N] need to be removed later
    // image size
    const uint32_t image_width,
    const uint32_t image_height,
    const uint32_t tile_size,
    ...
    GSPLAT_CHECK_INPUT(colors);
    GSPLAT_CHECK_INPUT(anomals); // need to be removed later
    uint32_t channels = colors.size(-1);
