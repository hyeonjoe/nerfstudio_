...
__global__ void custom_rasterize_to_pixels_bwd_kernel(
    const uint32_t C,
    const uint32_t N,
    ...
    const S *__restrict__ anomals,   // [C, N] or [nnz]
    ...
    const S *__restrict__ v_render_anomals, // [C, image_height, image_width, 1]
    // grad inputs
    ...
    S *__restrict__ v_anomals // [C, N] or [nnz]
) {
    ...
    v_render_anomals += camera_id * image_height * image_width;
...
call_kernel_with_dim(
    ...
    const torch::Tensor &anomals,                 // [C, N] or [nnz]
    ...
    const torch::Tensor &v_render_anomals, // [C, image_height, image_width, 1]
    // options
    bool absgrad
) {
    ...
    torch::Tensor v_anomals = torch::zeros_like(anomals);
    ...
    if (n_isects) {
        ...
        if (cudaFuncSetAttribute(
                custom_rasterize_to_pixels_bwd_kernel<CDIM, float>,
        ...
        custom_rasterize_to_pixels_bwd_kernel<CDIM, float>
            <<<blocks, threads, shared_mem, stream>>>(
                ...
                v_render_anomals.data_ptr<float>(),
                ...
                v_anomals.data_ptr<float>()
            );
    }

    return std::make_tuple(
        v_means2d_abs, v_means2d, v_conics, v_colors, v_opacities,v_anomals
    );
}
...
std::tuple<
    ...
    torch::Tensor,
    torch::Tensor>
custom_rasterize_to_pixels_bwd_tensor(
    // Gaussian parameters
    const torch::Tensor &means2d,                   // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,                    // [C, N, 3] or [nnz, 3]
    const torch::Tensor &colors,                    // [C, N, 3] or [nnz, 3]
