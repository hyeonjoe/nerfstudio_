...
def fully_fused_projection(
    ...
    if packed:
        return _FullyFusedProjectionPacked.apply(
            ...
            far_plane,
            radius_clip,
            sparse_grad,#difference btw packed and not packed
            calc_compensations,
            camera_model,
        )
    else:
...
def custom_rasterize_to_pixels(
    means2d: Tensor,  # [C, N, 2] or [nnz, 2]
    conics: Tensor,  # [C, N, 3] or [nnz, 3]
    colors: Tensor,  # [C, N, channels] or [nnz, channels]
    opacities: Tensor,  # [C, N] or [nnz]
    anomals : Tensor,  # [nnz] need to be removed later
    image_width: int,
    image_height: int,
    tile_size: int,
    isect_offsets: Tensor,  # [C, tile_height, tile_width]
    ...
    render_colors, render_alphas,render_anomals = _custom_RasterizeToPixels.apply(
        means2d.contiguous(),
        conics.contiguous(),
        colors.contiguous(),
        opacities.contiguous(),
    ...
    if padded_channels > 0:
        render_colors = render_colors[..., :-padded_channels]
    return render_colors, render_alphas,render_anomals

class _custom_RasterizeToPixels(torch.autograd.Function):
    """Rasterize gaussians"""
    ...
    def forward(
        ...
        backgrounds: Tensor,  # [C, D], Optional
        masks: Tensor,  # [C, tile_height, tile_width], Optional
        anomals: Tensor,  # [C, tile_height, tile_width] need to be removed later
        width: int,
        height: int,
        tile_size: int,
        isect_offsets: Tensor,  # [C, tile_height, tile_width]
        flatten_ids: Tensor,  # [n_isects]
        absgrad: bool,
    ) -> Tuple[Tensor, Tensor]: #need to be removed later
        render_colors, render_alphas, last_ids,render_anomals = _make_lazy_cuda_func(
